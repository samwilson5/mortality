...
create.Climate.rcp = function(sit,yearStart,yearEnd) {
  ...
  if (!inherits(site.data, "try-error")) {
    ...
    colnames(xdf) <- c("Year", "DOY", "Tmax_C", "Tmin_C", "PPT")
...
# going to need to adjust the input file for 4.5/8.5 gcms!
gcms=c('bcc-csm1-1','bcc-csm1-1-m','BNU-ESM','CanESM2','CCSM4','CNRM-CM5',
...
for(j in rcps){
  for(i in gcms){
    file = paste0("input_files/future/all_sites_",i,"_",j,".csv")
    livneh = read.csv(file)
    for(k in 1:898){
      sit = labels[k]
      ...
      veg = create.Veg(wdata,L1,L2,grass,slope,as.integer(2040),as.integer(2060))
      ...
      else if(j=='85'){co2=data.frame(Year=2040:2060,CO2ppm=600:620)}
      veg$sw_in@carbon@CO2ppm = as.matrix(co2)
      historic_run = run_SOILWAT2(2040,soil_outputs$all_soils,veg$trco,veg$veg_cover,
    ...
    write.csv(data,paste0('output_files/future2/',i,"_",j,'_drydaysTEMP_midCentury.csv'))}
}
...
for(j in rcps){
  for(i in gcms){
    ...
    for(k in 1:898){
      ...
      wdata = create.Climate.rcp(sit,2070,2090)
      veg = create.Veg(wdata,L1,L2,grass,slope,as.integer(2070),as.integer(2090))
      soil_outputs = create.Soil(gravel_in,sand_in,clay_in,sw_in)
      #co2 = read.csv('input_files/CO2_historic.csv')
      #co2 = co2 %>% rename(CO2ppm = CO2,Year = year) %>% select(Year,CO2ppm)
      #co2 = co2 %>% filter(Year != 1914)
      if(j=='45'){co2=data.frame(Year=2070:2090,CO2ppm=650:670)}
      else if(j=='85'){co2=data.frame(Year=2070:2090,CO2ppm=800:820)}
      veg$sw_in@carbon@CO2ppm = as.matrix(co2)
      historic_run = run_SOILWAT2(2070,soil_outputs$all_soils,veg$trco,veg$veg_cover,
                                  veg$sw_in,wdata,soil_outputs$VWC,
                                  soil_outputs$AET,soil_outputs$SWP)
      matrix = as.data.frame(rSOILWAT2::dbW_weatherData_to_dataframe(wdata))
      size = nrow(matrix)
      ...
      data = left_join(data,dat,join_by(Year==Year))
      data$plot = sit
      #write.csv(data,paste0('output_files/future/',i,"_",j,'_drydaysTEMP_lateCentury.csv'))
      if(k==1){out = data}
      else{out = rbind(out,data)}
    }
    write.csv(data,paste0('output_files/future2/',i,"_",j,'_drydaysTEMP_lateCentury.csv'))
    }
  }
